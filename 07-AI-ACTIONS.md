...
2. **Configuración Básica**
   ```yaml
   Name: search_koaj_products
   Display Name: "Buscar Productos KOAJ"
...
```json
{
  "action_config": {
    ...
    "parameters": {
      "required": ["query"],
      "optional": ["category", "price_range", "size", "color"],
      "extraction_rules": {
        "query": {
          "type": "string",
          "source": "user_message",
    ...
    "execution": {
      "endpoint": "https://api.neero.link/v1/bird/ai-search",
      "method": "POST",
      "headers": {
...
```yaml
Webhook Configuration:
  URL: https://api.neero.link/v1/bird/ai-search
...
```json
{
  ...
  "filters": {
    ...
    "price_range": {
      "min": {{extracted.price_min | default: 50000}},
      "max": {{extracted.price_max | default: 5000000}}
    },
    "size": "{{extracted.size | default: ''}}",
    "color": "{{extracted.color | default: ''}}",
    "brand": "KOAJ"
  },
  "options": {
...
class ParameterExtractor:
    ...
    def extract_price(self, text):
        patterns = {
            ...
            'budget_hints': {
                'económico': {'min': 50000, 'max': 200000},
                'barato': {'min': 50000, 'max': 150000},
                'premium': {'min': 500000, 'max': 5000000},
                'exclusivo': {'min': 800000, 'max': 5000000}
            }
        }
        ...
        for pattern_list in patterns['max_price']:
            ...
            if match:
                return {'min': 50000, 'max': int(match.group(1)) * 1000}
...
```yaml
Intent Mapping:
  user_says: "Busco un vestido negro para una cena"
...
class StatefulAction:
    ...
    def multi_step_action(self, step, params, conversation_id):
        ...
        if step == 'init':
            ...
            return {
                'message': '¿Qué tipo de prenda buscas?',
                'options': ['Vestidos', 'Blusas', 'Pantalones', 'Accesorios']
            }
        
        elif step == 'category_selected':
            state['filters']['category'] = params['selection']
            state['step'] = 'size_selection'
            return {
                'message': '¿Cuál es tu talla?',
                'options': ['XS', 'S', 'M', 'L', 'XL']
            }
        
        elif step == 'size_selected':
            state['filters']['size'] = params['selection']
            state['step'] = 'search'
